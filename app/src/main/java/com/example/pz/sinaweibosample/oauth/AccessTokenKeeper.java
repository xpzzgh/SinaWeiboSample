...
public class AccessTokenKeeper {
    ...
    public static void writeToken(Oauth2AccessToken mAccessToken) {
        if(mAccessToken == null) {
            return;
        }

        SharedPreferences pref = MyApplication.getContext().getSharedPreferences(PREFERENCES_NAME, Context.MODE_PRIVATE);
    ...
    public static void clear() {
        SharedPreferences.Editor editor = MyApplication.getContext().getSharedPreferences(PREFERENCES_NAME, Context.MODE_PRIVATE).edit();
        editor.clear();
        editor.commit();
    }

    public static boolean isTokenValid() {
        Oauth2AccessToken mAccessToken = readToken();
